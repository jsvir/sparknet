...
class ASRModel(ModelPT, ABC):
    ...
    def multi_validation_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        if self.gates_prediction:
            val_loss_gates_mean = torch.stack([x['val_loss_gates'] for x in outputs]).mean()
            wer_num_gates = torch.stack([x['val_wer_num_gates'] for x in outputs]).sum()
            wer_denom_gates = torch.stack([x['val_wer_denom_gates'] for x in outputs]).sum()
            tensorboard_logs.update(
                {'val_loss_gates': val_loss_gates_mean, 'val_wer_gates': wer_num_gates / wer_denom_gates})
            return {'val_loss': val_loss_mean, 'val_loss_gates': val_loss_gates_mean, 'log': tensorboard_logs}
    ...
    def multi_test_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        if self.gates_prediction:
            val_loss_gates_mean = torch.stack([x['test_loss_gates'] for x in outputs]).mean()
            wer_num_gates = torch.stack([x['test_wer_num_gates'] for x in outputs]).sum()
            wer_denom_gates = torch.stack([x['test_wer_denom_gates'] for x in outputs]).sum()
            tensorboard_logs.update(
                {'test_loss_gates': val_loss_gates_mean, 'test_wer_gates': wer_num_gates / wer_denom_gates})
            return {'test_loss': val_loss_mean, 'test_loss_gates': val_loss_gates_mean, 'log': tensorboard_logs}
